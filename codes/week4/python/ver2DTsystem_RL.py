...
class FirstOrderPlant:
    ...
    def reset(self) -> Tuple[float, int, int]:
        """Reset plant to x=0, reset prev_u, and state at t=0."""
        x0 = 0.0
        self.prev_u = 0.0
...
# --- Agent with ε-greedy, end-of-episode planning, and configurable delta tolerance ---
class Agent:
    def __init__(
        self,
        num_states: int,
        ...
        self.transition_counts = np.zeros((num_states, num_actions, num_states), dtype=int)
        self.reward_sums = np.zeros((num_states, num_actions, num_states), dtype=float)
    ...
    def train_end_episode(self):
        # Policy Evaluation with configurable tolerance
        for _ in range(self.eval_max_iter):
            delta = 0.0
            for s in range(self.num_states):
                ...
                if total == 0:
                    continue
                P = counts / total
                R_avg = self.reward_sums[s, a].sum() / total
                v_new = np.dot(P, R_avg + self.gamma * self.value)
                delta = max(delta, abs(v_new - self.value[s]))
                self.value[s] = v_new
        ...
        for s in range(self.num_states):
            ...
            for a in range(self.num_actions):
                ...
                if total == 0:
                    ...
                    P = counts / total
                    R_avg = self.reward_sums[s, a].sum() / total
                    val = np.dot(P, R_avg + self.gamma * self.value)
                if val > best_val:
                    best_val = val
                    best_a = a
...
if __name__ == "__main__":
    ...
    env = FirstOrderPlant(
        ...
        error_weight=500.0,  # now configurable
        lam=0.01, beta=20.0
    )
    agent = Agent(
        env.num_states, env.num_actions,
        gamma=0.99, epsilon=0.9, min_epsilon=0.01, decay=0.995,
        delta_tol=1e-7, eval_max_iter=100
    )
