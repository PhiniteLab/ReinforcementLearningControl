...
# ----- Gym'e alternatif CartPole ortamı -----
class CustomCartPoleEnv:
    ...
    def reset(self, seed=None):
        if seed is not None:
            np.random.seed(seed)
        self.state = np.random.uniform(low=[-0.05, -0.05, -0.05, -0.05],
                                       high=[0.05, 0.05, 0.05, 0.05])
        self.step_count = 0
        return self.state.copy()

    def step(self, action):
        assert action in [0, 1], "Action 0 (left) or 1 (right) olmalı."
        state = self.state
...
def hyperparameter_search():
    env = CustomCartPoleEnv(max_episode_steps=2000)
    alphas = [0.21, 0.19, 0.2]
    gammas = [0.99, 0.999]
    epsilons = [1.0]
    epsilon_decays = [0.999]
    min_epsilons = [0.01, 0.02, 0.04]
    num_episodes = 10000
    num_test_episodes = 5
