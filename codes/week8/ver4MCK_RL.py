...
class MassSpringDamperEnv:
    ...
    def reset(self, seed=None):
        if seed is not None:
            np.random.seed(seed)
        # Başlangıçta küçük sapmalar
        self.state = np.random.uniform(low=[-0.05, -0.05], high=[0.05, 0.05])
        self.step_count = 0
        return self.state.copy()

    def step(self, action):
        # Action: [0, 1, 2] -> [sol kuvvet, kuvvet yok, sağ kuvvet]
        if isinstance(action, np.ndarray):
            action = int(action)
        force = [-self.u_max, 0.0, self.u_max][action]
        x, x_dot = self.state

        # Euler forward ile güncelleme
        ...
        # Sınırdan çıkarsa bitir
        done = bool(
            abs(x) > self.x_threshold or self.step_count >= self.max_episode_steps
        )

        # Reward: referansa yakınlık, ek olarak ceza ve/veya bonus verilebilir
        reward = -abs(x - self.ref)  # L1 hata
...
class FastMCKControl:
    def __init__(self, alpha, gamma, epsilon, epsilon_decay=0.999, min_epsilon=0.01):
        ...
        # 3 eylem: sola kuvvet, kuvvet yok, sağa kuvvet
...
def animate_mck(agent, env, max_steps=500, max_trials=3):
    for trial in range(max_trials):
        ...
        while not done and steps < max_steps:
            ...
            force = [-env.u_max, 0.0, env.u_max][action]
...
def hyperparameter_search():
    ...
    # === EN İYİ PARAMETRELERLE 10000 EPISODE EĞİTİM ===
    print("\n=== EN İYİ AĞ İLE 10.000 EPISODE YENİDEN EĞİTİM BAŞLIYOR ===")
    best_params = best['params']
    agent = FastMCKControl(*best_params)
    agent.learn(env, num_episodes=10000)
    save_agent(agent, "best_mck_qtable_10k.npy")
    print("\n10.000 episode sonrası TEST:")
    agent.test(env, num_episodes=5)
    print("\n10.000 episode sonrası ANİMASYON:")
    animate_mck(agent, env, max_steps=300, max_trials=2)

if __name__ == "__main__":
    # Hiperparametre araması ve en iyi agent'ı kaydet + 10.000 ek eğitim
    hyperparameter_search()

    # Kayıtlı agent'ı yükle ve test/animasyon yap (10.000 episode eğitilmiş Q-table ile)
    env = MassSpringDamperEnv(max_episode_steps=300, ref=1.0)
    loaded_agent = FastMCKControl(alpha=0.15, gamma=0.999, epsilon=0.01)
    load_agent(loaded_agent, "best_mck_qtable_10k.npy")
